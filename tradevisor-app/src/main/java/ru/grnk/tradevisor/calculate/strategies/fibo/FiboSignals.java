...
public class FiboSignals implements IStrategy {
    ...
    private static LevelAnalysisResult analyzeLevels(
        ...
        TouchCountResult touches382 = countTouches(tickerData, extremums, fiboLevels.sigma(), fiboLevels.fibo382());
        TouchCountResult touches618 = countTouches(tickerData, extremums, fiboLevels.sigma(), fiboLevels.fibo618());
    ...
    private static TouchCountResult countTouches(
        ...
        for (int j = 0; j < extremums.right().index(); j++) {
            boolean touched = isLevelTouched(tickerData.get(j), extremums.trend(), levelValue, sigma);
            if (touched) {
                touches++;
